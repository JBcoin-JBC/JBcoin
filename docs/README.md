...
2. Build Docker image. From the jbcoind root folder:

```
sudo docker build -t jbcoind-docs docs/
```

Then to run the image, from the jbcoind root folder:

```
sudo docker run -v $PWD:/opt/jbcoind --rm jbcoind-docs
```
