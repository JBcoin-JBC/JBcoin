...
TER PathCursor::advanceNode (STAmount const& amount, bool reverse, bool callerHasLiquidity) const
{
    ...
    PathCursor withMultiQuality {jbcoinCalc_, pathState_, multi, j_, nodeIndex_};
        ...
        else
        {
            ...
            else
            {
                            ...
                            view().parentCloseTime().time_since_epoch().count()))
                {
                    ...
                    jbcoinCalc_.permanentlyUnfundedOffers_.insert(
                        node().offerIndex_);
                    continue;
                }
                ...
                if (node().saTakerPays <= beast::zero || node().saTakerGets <= beast::zero)
                {
                    ...
                    else if (jbcoinCalc_.permanentlyUnfundedOffers_.find (index)
                             != jbcoinCalc_.permanentlyUnfundedOffers_.end ())
                    {
                ...
                auto itPast = jbcoinCalc_.mumSource_.find (accountIssue);
                bool bFoundPast = (itPast != jbcoinCalc_.mumSource_.end ());
                ...
                if (node().saOfferFunds <= beast::zero)
                {
                    ...
                    if (bReverse && !bFoundReverse && !bFoundPast)
                    {
                        ...
                        // still do deletions.
                        // Mark offer for always deletion.
                        jbcoinCalc_.permanentlyUnfundedOffers_.insert (node().offerIndex_);
                    }
                    else
                    {
