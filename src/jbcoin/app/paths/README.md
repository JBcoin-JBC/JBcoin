...
This is the **jbcoind** implementation; there are many other possible implementations!

**jbcoind** uses a variety of search techniques:

1. Hebbian learning.
 * Reuse found liquidity.
 * Good liquidity is often reusable.
 * When searching, limit our search to the jbcoind cache.
 ...
 * By using JBC for bridging the most complicated path expected is usually:
 * source -> gateway -> JBC -> gateway -> destination
3. Pareto principle.
 * Good liquidity is often reusable.
 ...
 * Learn new paths.
 * Search outside the **jbcoind** cache.
 * Distributed learning.
 * Every jbcoind node searches their own cache for best liquidity and then the good results get propagated to the network.
 * Whenever somebody makes a payment, the nodes involved go to the jbcoind cache; since payments appear in every ledger in the network, this liquidity information now appears in every jbcoind cache.
...
Making a payment in the jbcoin network is about finding the cheapest path between source and destination.
...
For tx processing, people submit a tx to a jbcoind node, which attempts to apply the tx locally first, and if succeesful, distributes it to other nodes.

When someone accepts payment they list their specific payment terms, "what must happen before the payment goes off."  That can be done completely in the JBCoin Network.  Normally a payment on the JBCoin net can be completely settled there.  When the ledger closes, the terms are met, or they are not met.
...
In contrast, a gateway is in the business of redeeming value on the JBCoin Network for value off the Ripnet.  A gateway is in the business of issuing balances.

Bitstamp is a gateway - you send 'em cash, they give you a jbcoin balance and vice versa.  There's no promise or forwarding for a transaction.

A bridge is a facility that allows payments to be made from the Ripnet to off the Ripnet.

Suppose I'm on the JBCoin network and want to send value to the bitcoin network.  See:  https://jbcoin.com/wiki/Outbound_Bridge_Payments
https://jbcoin.com/wiki/Services_API
...
https://jbcoin.com/wiki/Transaction_Format#Payment_.280.29
...
1. I make a request for path finding to a jbcoind.
2. That jbcoind "continuously" returns solutions until I "stop".
3. You create a transaction which includes the path set that you got from the quote.
4. You sign it.
5. You submit it to one or more jbcoinds.
6. Those jbcoind validates it, and forwards it if it;s valid.
...
If you're sending and receiving JBC you don't need a path.
...
1. Finding liquidity - "path finding"
  ...
  * filtering by liquidity.
    ...
    * This routine is called JBCoinCalc::jbcoinCalc.
...
2. Build a payment transaction containing the path set.
  ...
  * A payment transaction includes the following fields
    ...
    * destination amount
      ...
      * if currency not JBC, must have a specified issuer.
 ...
 * The routine used to compute liquidity and ledger change is also called JBCoinCalc::jbcoinCalc.
