...
The _JBCoin payment network_ consists of a collection of _peers_ running
**jbcoind**. Each peer maintains multiple outgoing connections and optional
...
fully connected directed graph of nodes where vertices are instances of jbcoind
...
establishment, defines the _JBCoin peer protocol_ (_protocol_ in this context).
...
* _User Defined_ (Unimplemented)
    ...
    The jbcoind operator may specify additional, optional fields and values
    through the configuration. These headers will be transmitted in the
    corresponding request or response messages.

# JBCoin Clustering #

A cluster consists of more than one JBCoin server under common
...
Cluster members are configured in the `jbcoind.cfg` file under
