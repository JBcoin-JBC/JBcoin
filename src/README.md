...
into the `jbcoind` repository. They can be edited and committed just as if they
were normal files.
[git-subtree]: https://github.com/apenwarr/git-subtree

If you create a commit that contains files both from a subtree, and from the
`jbcoind` source tree, please use care when designing the commit message, since
...
`jbcoin` in the same commit at all.
...
| `protobuf`      | https://github.com/google/protobuf           | Protocol buffer data interchange format. JBCoin has changed some names in order to support the unity-style of build (a single .cpp added to the project, instead of linking to a separately built static library). |
| `jbcoin`        | N/A                                          | **Core source code for `jbcoind`** |
...
| `test`          | N/A                                          | **Unit tests for `jbcoind`** |
