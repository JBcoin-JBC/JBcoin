...
#ifndef JBCOIN_TEST_JTX_ENV_H_INCLUDED
#define JBCOIN_TEST_JTX_ENV_H_INCLUDED
...
/** Designate accounts as no-jbcoin in Env::fund */
template <class... Args>
std::array<Account, 1 + sizeof...(Args)>
nojbcoin (Account const& account, Args const&... args)
{
    return {{account, args...}};
}
...
supported_amendments()
{
    static const FeatureBitset ids = []{
        auto const& sa = jbcoin::detail::supportedAmendments();
...
public:
    ...
    /** Returns the current JBCoin Network Time
    ...
    */
    ...
    /** Returns the JBC balance on an account.
...
public:
    ...
    /** Create a new account with some JBC.
        ...
        Effects:
            The asfDefaultJBCoin on the account is set,
            and the sequence number is incremented, unless
            the account is wrapped with a call to nojbcoin.

            The account's JBC balance is set to amount.

            Generates a test that the balance is set.

        @param amount The amount of JBC to transfer to
                      each account.

        @param args A heterogeneous list of accounts to fund
                    or calls to nojbcoin with lists of accounts
                    to fund.
    */
