...
public:
    void testErrors()
    {
       ...
       // test errors on marker
        {
            Account const gw{ "G" };
            env.fund(JBC(1000), gw);
            auto const USD = gw["USD"];
            env.trust(USD(1000), bob);
            env(pay(gw, bob, JBC(1)));
            env(offer(bob, JBC(100), bob["USD"](1)), txflags(tfPassive));
    ...
    void testUnsteppedThenStepped()
    {
        ...
        env(offer(bob, JBC(100), bob["USD"](1)),txflags(tfPassive));
        env(offer(bob, JBC(100), USD1(1)), txflags(tfPassive));
    ...
    void testObjectTypes()
    {
        ...
        }
        {
            ...
            jvEscrow[jss::Amount] = JBC(100).value().getJson(0);
        ...
        }
        {
            ...
            jvPayChan[jss::Amount] = JBC (300).value().getJson (0);
...
BEAST_DEFINE_TESTSUITE(AccountObjects,app,jbcoin);
