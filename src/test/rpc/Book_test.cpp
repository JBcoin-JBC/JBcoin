...
public:
    ...
    testOneSideEmptyBook()
    {
...

        {
            ...
            books[jss::books] = Json::arrayValue;
            {
                ...
                j[jss::snapshot] = true;
                j[jss::taker_gets][jss::currency] = "JBC";
            ...
            if (wsc->version() == 2)
            {
                ...
                BEAST_EXPECT(jv.isMember(jss::jbcoinrpc) && jv[jss::jbcoinrpc] == "2.0");
...

        {
            ...
            env(offer("alice", JBC(700), USD(100)),
                ...
                [&](auto const& jv)
                {
                    ...
                    return t[jss::TransactionType] == "OfferCreate" &&
                        t[jss::TakerGets] == USD(100).value().getJson(0) &&
                            t[jss::TakerPays] == JBC(700).value().getJson(0);
                }));
        }

        {
            // Create a bid: TakerPays 100/USD, TakerGets 75
            env(offer("alice", USD(100), JBC(75)),
    ...
    testOneSideOffersInBook()
    {
        ...
        env(offer("alice", USD(100), JBC(200)),
            require(owners("alice", 2)));
        env.close();
...

        {
            ...
            BEAST_EXPECT(wsc->findMsg(5s,
                [&](auto const& jv)
                {
                    auto const& t = jv[jss::transaction];
                    return t[jss::TransactionType] == "OfferCreate" &&
                        t[jss::TakerGets] == JBC(75).value().getJson(0) &&
    ...
    testTrackOffers()
    {
        ...
        Account bob {"bob"};
        auto wsc = makeWSClient(env.app().config());
        env.fund(JBC(20000), alice, bob, gw);
        env.close();
        auto USD = gw["USD"];
        ...
        env(offer(alice, JBC(4000), USD(10)));
        env.close();

        Json::Value jvParams;
        jvParams[jss::taker] = env.master.human();
        jvParams[jss::taker_pays][jss::currency] = "JBC";
        ...
        BEAST_EXPECT(jrOffer[jss::TakerPays] == JBC(4000).value().getJson(0));
        ...
        BEAST_EXPECT(wsc->findMsg(5s,
            [&](auto const& jv)
            {
                ...
                return t[jss::TransactionType] == "OfferCreate" &&
                       ...
                       t[jss::TakerPays] == JBC(4000).value().getJson(0);
            }));

        env(offer(bob, JBC(2000), USD(5)));
        env.close();
        ...
        BEAST_EXPECT(wsc->findMsg(5s,
            [&](auto const& jv)
            {
                ...
                return t[jss::TransactionType] == "OfferCreate" &&
                       ...
                       t[jss::TakerPays] == JBC(2000).value().getJson(0);
            }));
        ...
        BEAST_EXPECT(jrNextOffer[jss::TakerPays] == JBC(2000).value().getJson(0));
    ...
    testCrossingSingleBookOffer()
    {
        ...
        // Scenario is:
        //  - Alice and Bob place identical offers for USD -> JBC
        ...
        env.fund (JBC(1000000), gw, alice, bob, charlie);
        env.close();
        ...
        // Alice and Bob offer $500 for 500 JBC
        env (offer (alice, JBC(500), USD(500)));
        env (offer (bob, JBC(500), USD(500)));
        env.close();
        ...
        env(offer(charlie, USD(1000), JBC(1000)));
        env.close();
        env.require(offers(alice, 0), offers(bob, 0), offers(charlie, 0));
        using namespace std::chrono_literals;
        BEAST_EXPECT(offerOnlyOnceInStream(wsc, 1s, JBC(1000), USD(1000)));
    ...
    testCrossingMultiBookOffer()
    {
        ...
        // Scenario is:
        //  - Alice has 1 USD and wants 100 JBC
        //  - Bob has 100 JBC and wants 1 EUR
        ...
        env(offer(alice, JBC(100), USD(1)));
        env(offer(bob, EUR(1), JBC(100)));
        env.close();
...

        {
...

            {
                ...
                j[jss::taker_pays][jss::currency] = "JBC";
            }
    ...
    testBookOfferErrors()
    {
...

        {
            ...
            jvParams[jss::taker_gets][jss::currency] = "JBC";
            auto const jrr = env.rpc(
                "json", "book_offers", to_string(jvParams)) [jss::result];
...

        {
            ...
            BEAST_EXPECT(jrr[jss::error_message] ==
                ...
                "for JBC currency specification.");
        }

        {
            ...
            jvParams[jss::taker_pays][jss::issuer]   = toBase58(jbcAccount());
            jvParams[jss::taker_gets][jss::currency] = "USD";
            jvParams[jss::taker_gets][jss::issuer]   = gw.human();
            auto const jrr = env.rpc(
                "json", "book_offers", to_string(jvParams)) [jss::result];
    ...
    testBookOfferLimits(bool asAdmin)
    {
        ...
        Account gw {"gw"};
        env.fund(JBC(200000), gw);
        env.close();
        auto USD = gw["USD"];

        for(auto i = 0; i <= RPC::Tuning::bookOffers.rmax; i++)
            env(offer(gw, JBC(50 + 1*i), USD(1.0 + 0.1*i)));
        env.close();
...
BEAST_DEFINE_TESTSUITE_PRIO(Book,app,jbcoin,1);
