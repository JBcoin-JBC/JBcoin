...
public:
    void testAccountLines()
    {
        ...
        for (char c = 0; c <= ('Z' - 'A'); ++c)
        {
            ...
            env(trust(alice, gw2Currency(0), gw2, tfSetNoJBCoin | tfSetFreeze));
        }
        env.close();
        ...
        }
        {
            ...
            BEAST_EXPECT(line[jss::no_jbcoin].asBool() == true);
        ...
        }
        {
            ...
            BEAST_EXPECT(lineA[jss::no_jbcoin_peer].asBool() == true);
    ...
    void testAccountLineDelete()
    {
        ...
        //   o alice offers to buy 100 USD for 100 JBC.
        //   o becky offers to sell 100 USD for 100 JBC.
        ...
        Account const gw1 {"gw1"};
        Account const gw2 {"gw2"};
        env.fund(JBC(10000), alice, becky, cheri, gw1, gw2);
        env.close();
        ...
        // alice offers to buy 100 EUR for 100 JBC.
        env(offer(alice, EUR(100), JBC(100)));
        env.close();

        // becky offers to buy 100 JBC for 100 EUR.
        env(offer(becky, JBC(100), EUR(100)));
        env.close();
    ...
    void testAccountLines2()
    {
        ...
        Env env(*this);
        {
            ...
            BEAST_EXPECT(lines.isMember(jss::jbcoinrpc) && lines[jss::jbcoinrpc] == "2.0");
        }
        {
            // account_lines with no account.
            auto const lines = env.rpc ("json2", "{ "
        ...
        [this, &env](Account const& account, LedgerInfo const& info, int count)
        {
            ...
            BEAST_EXPECT(linesSeq.isMember(jss::jbcoinrpc) && linesSeq[jss::jbcoinrpc] == "2.0");
            ...
            BEAST_EXPECT(linesHash.isMember(jss::jbcoinrpc) && linesHash[jss::jbcoinrpc] == "2.0");
        ...
        }
        {
            ...
            BEAST_EXPECT(linesA.isMember(jss::jbcoinrpc) && linesA[jss::jbcoinrpc] == "2.0");
            ...
            BEAST_EXPECT(linesB.isMember(jss::jbcoinrpc) && linesB[jss::jbcoinrpc] == "2.0");
            ...
            BEAST_EXPECT(linesC.isMember(jss::jbcoinrpc) && linesC[jss::jbcoinrpc] == "2.0");
            ...
            BEAST_EXPECT(linesD.isMember(jss::jbcoinrpc) && linesD[jss::jbcoinrpc] == "2.0");
    ...
    void testAccountLineDelete2()
    {
        ...
        //   o alice offers to buy 100 EUR for 100 JBC.
        //   o becky offers to sell 100 EUR for 100 JBC.
        ...
        BEAST_EXPECT(linesBeg.isMember(jss::jbcoinrpc) && linesBeg[jss::jbcoinrpc] == "2.0");
        ...
        BEAST_EXPECT(linesEnd.isMember(jss::jbcoinrpc) && linesEnd[jss::jbcoinrpc] == "2.0");
...
BEAST_DEFINE_TESTSUITE(AccountLinesRPC,app,jbcoin);
