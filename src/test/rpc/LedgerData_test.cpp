...
public:
    ...
    void testLedgerType()
    {
        ...
        env(offer(Account{"bob0"}, USD(100), JBC(100)));
...

        {
            ...
            jv[jss::Amount] = JBC(50).value().getJson(0);
...

        {
            ...
            jv[jss::Amount] = JBC(100).value().getJson (0);
        ...
        BEAST_EXPECT( checkArraySize(jrr[jss::state], 1) );
        for (auto const& j : jrr[jss::state])
            BEAST_EXPECT( j["LedgerEntryType"] == "JBCoinState" );
        }
...
BEAST_DEFINE_TESTSUITE_PRIO(LedgerData,app,jbcoin,1);
