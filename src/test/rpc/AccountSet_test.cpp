...
public:
    ...
    void testMostFlags()
    {
        ...
        testFlags ({asfRequireDest, asfRequireAuth, asfDisallowJBC,
            asfGlobalFreeze, asfDisableMaster, asfDefaultJBCoin});
    ...
    void testBadInputs(bool withFeatures)
    {
        ...
        auto jt = fset(alice, asfDisallowJBC);
        jt[jss::ClearFlag] = asfDisallowJBC;
        ...
        jt = fset(alice, asfDisallowJBC);
        jt[sfFlags.fieldName] = tfAllowJBC;
        env(jt, ter(temINVALID_FLAG));
...
BEAST_DEFINE_TESTSUITE(AccountSet,app,jbcoin);
