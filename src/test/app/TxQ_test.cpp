...
class TxQ_test : public beast::unit_test::suite
{
        ...
        std::uint32_t units, std::uint32_t reserve, std::uint32_t increment)
    {
        ...
        checkMetrics(env, 0,
            2 * (jbcoin::detail::supportedAmendments().size() + 1),
...
public:
    void testQueue()
    {
        ...
        env.fund(JBC(50000), nojbcoin(alice, bob, charlie, daria));
        ...
        env.fund(JBC(50000), nojbcoin(elmo, fred, gwen, hank));
    ...
    void testTecResult()
    {
        ...
        // Create accounts
        env.fund(JBC(50000), nojbcoin(alice, gw));
        ...
        env(offer(alice, JBC(1000), USD(1000)), ter(tecUNFUNDED_OFFER));
        ...
        env(offer(alice, JBC(1000), USD(1000)), ter(terQUEUED));
    ...
    void testLocalTxRetry()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(JBC(50000), nojbcoin(alice, bob, charlie));
    ...
    void testLastLedgerSeq()
    {
        ...
        env.fund(JBC(1000), nojbcoin(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(JBC(1000), nojbcoin(charlie, daria));
        env.close(env.now() + 5s, 10000ms);
        env.fund(JBC(1000), nojbcoin(edgar, felicia));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testZeroFeeTxn()
    {
        ...
        env.fund(JBC(1000), nojbcoin(alice, bob));
        env.close(env.now() + 5s, 10000ms);
        env.fund(JBC(1000), nojbcoin(carol));
        env.close(env.now() + 5s, 10000ms);
    ...
    void testPreclaimFailures()
    {
        ...
        env.fund(JBC(1000), nojbcoin(alice));
        ...
        // Fail in preflight
        env(pay(alice, bob, JBC(-1000)),
            ter(temBAD_AMOUNT));

        // Fail in preclaim
        env(noop(alice), fee(JBC(100000)),
            ter(terINSUF_FEE_B));
    }
    ...
    void testMultiTxnPerAccount()
    {
        ...
        // Create several accounts while the fee is cheap so they all apply.
        env.fund(drops(2000), nojbcoin(alice));
        env.fund(JBC(500000), nojbcoin(bob, charlie, daria));
        ...
        aliceFee = env.le(alice)->getFieldAmount(sfBalance).jbc().drops()
            - (59);
        env(noop(alice), seq(aliceSeq),
        ...
        // Alice is broke
        env.require(balance(alice, JBC(0)));
    ...
    void testDisabled()
    {
        ...
        env.fund(JBC(50000), nojbcoin(alice));
    ...
    void testUnexpectedBalanceChange()
    {
        ...
        env.fund(drops(5000), nojbcoin(alice));
        env.fund(JBC(50000), nojbcoin(bob));
        ...
        /* At this point, Alice's transaction is indefinitely
            ...
            will get more JBC, and it'll process.
        */
    ...
    void testBlockers()
    {
        ...
        env.fund(JBC(50000), nojbcoin(alice, bob));
        env.memoize(charlie);
        env.memoize(daria);
    ...
    void testInFlightBalance()
    {
        ...
        env.fund(JBC(50000), nojbcoin(alice, charlie), gw);
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        //////////////////////////////////////////
        // Offer with high JBC out blocks later txs
        auto aliceSeq = env.seq(alice);
        auto aliceBal = env.balance(alice);

        env.require(balance(alice, JBC(50000)),
            owners(alice, 0));

        // If this offer crosses, all of alice's
        // JBC will be taken (except the reserve).
        env(offer(alice, BUX(5000), JBC(50000)),
            queued);
        ...
        // has plenty of JBC, because the offer didn't
        // cross (of course).
        env.require(balance(alice, aliceBal - drops(20)),
            owners(alice, 1));

        //////////////////////////////////////////
        // Offer with low JBC out allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 8, 5, 4, 256);
        aliceSeq = env.seq(alice);
        ...
        // of alice's JBC will be taken.
        env(offer(alice, BUX(50), JBC(500)),
            queued);
        ...
        //////////////////////////////////////////
        // Large JBC payment blocks later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 10, 6, 5, 256);
        ...
        // (minus the reserve).
        env(pay(alice, charlie, JBC(50000)),
            queued);
        ...
        //////////////////////////////////////////
        // Small JBC payment allows later txs
        fillQueue(env, alice);
        checkMetrics(env, 0, 12, 7, 6, 256);
        ...
        env.require(balance(alice, aliceBal - JBC(500) - drops(20)),
            owners(alice, 2));
        ...
        // alice's JBC balance (other than the fee, of course).
        env(noop(alice), seq(aliceSeq + 1), queued);
        ...
        // JBC balance, but her USD balance went to charlie.
        env.require(balance(alice, aliceBal - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Large JBC to IOU payment blocks later txs.

        env(offer(gw, JBC(500000), USD(50000)));
        // Close so we don't have to deal
        // with tx ordering in consensus.
        env.close();
        ...
        // entire JBC balance to charlie in the
        // form of USD.
        BEAST_EXPECT(JBC(60000) > aliceBal);
        env(pay(alice, charlie, USD(1000)),
            sendmax(JBC(60000)), queued);
        ...
        // to charlie using only a portion of her JBC balance
        env.require(balance(alice, aliceBal - JBC(10000) - drops(20)),
            balance(alice, USD(0)),
        ...
        //////////////////////////////////////////
        // Small JBC to IOU payment allows later txs.
        ...
        // a portion of her JBC balance to charlie
        // in the form of USD.
        BEAST_EXPECT(aliceBal > JBC(6001));
        env(pay(alice, charlie, USD(500)),
            sendmax(JBC(6000)), queued);
        ...
        // So once we close the ledger, alice sent a payment
        // to charlie using only a portion of her JBC balance
        env.require(balance(alice, aliceBal - JBC(5000) - drops(20)),
            balance(alice, USD(0)),
    ...
    void testExpirationReplacement()
    {
        ...
        */
        ...
        env.fund(JBC(500000), nojbcoin(alice, bob));
        checkMetrics(env, 0, boost::none, 2, 1, 256);
    ...
    void testServerSubscribe()
    {
        ...
        env.fund(JBC(50000), nojbcoin(a,b,c,d));
        checkMetrics(env, 0, boost::none, 4, 3, 256);
        ...
        env.fund(JBC(50000), nojbcoin(e,f,g,h,i));
    ...
    void testClearQueuedAccountTxs()
    {
        ...
        checkMetrics(env, 0, boost::none, 0, 3, 256);
        env.fund(JBC(50000000), alice, bob);
        ...
        testcase("clear queue failure (load)");
        {
            ...
            env(noop(alice), fee(JBC(1)), seq(aliceSeq++), ter(terQUEUED));
...
BEAST_DEFINE_TESTSUITE_PRIO(TxQ,app,jbcoin,1);
