...
struct PayChan_test : public beast::unit_test::suite
{
    ...
    {
        Serializer msg;
        serializePayChanAuthorization (msg, channel, authAmt.jbc ());
    ...
    testSimple ()
    {
        ...
        env (create (alice, bob, JBC (1000), settleDelay, pk));
        auto const chan = channel (*env.current (), alice, bob);
        BEAST_EXPECT (channelBalance (*env.current (), chan) == JBC (0));
        BEAST_EXPECT (channelAmount (*env.current (), chan) == JBC (1000));

        {
            auto const preAlice = env.balance (alice);
            env (fund (alice, chan, JBC (1000)));
            auto const feeDrops = env.current ()->fees ().base;
            BEAST_EXPECT (env.balance (alice) == preAlice - JBC (1000) - feeDrops);
        }

        auto chanBal = channelBalance (*env.current (), chan);
        auto chanAmt = channelAmount (*env.current (), chan);
        BEAST_EXPECT (chanBal == JBC (0));
        BEAST_EXPECT (chanAmt == JBC (2000));

        {
            // bad amounts (non-jbc, negative amounts)
        ...
        // invalid account
        env (create (alice, "noAccount", JBC (1000), settleDelay, pk),
            ter (tecNO_DST));
        // can't create channel to the same account
        env (create (alice, alice, JBC (1000), settleDelay, pk),
             ter (temDST_IS_SRC));
        // invalid channel
        env (fund (alice, channel (*env.current (), alice, "noAccount"), JBC (1000)),
            ter (tecNO_ENTRY));
        // not enough funds
        env (create (alice, bob, JBC (10000), settleDelay, pk),
            ter (tecUNFUNDED));

        {
            // No signature claim with bad amounts (negative and non-jbc)
            auto const iou = USDA (100).value ();
            auto const negJBC = JBC (-100).value ();
            auto const posJBC = JBC (100).value ();
            env (claim (alice, chan, iou, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posJBC, iou), ter (temBAD_AMOUNT));
            env (claim (alice, chan, iou, posJBC), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negJBC, negJBC), ter (temBAD_AMOUNT));
            env (claim (alice, chan, posJBC, negJBC), ter (temBAD_AMOUNT));
            env (claim (alice, chan, negJBC, posJBC), ter (temBAD_AMOUNT));
        }
        {
            // No signature claim more than authorized
            auto const delta = JBC (500);
            auto const reqBal = chanBal + delta;
            auto const authAmt = reqBal + JBC (-100);
        ...
        }
        {
            ...
            auto const preBob = env.balance (bob);
            STAmount const authAmt = chanBal + JBC (500);
        ...
        env (fund (bob, chan, JBC (1000)), ter (tecNO_PERMISSION));
        BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
        BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);

        {
            // Wrong signing key
            auto const sig =
                signClaimAuth (bob.pk (), bob.sk (), chan, JBC (1500));
            env (claim (bob, chan, JBC (1500).value (), JBC (1500).value (),
                     Slice (sig), bob.pk ()),
                ter (temBAD_SIGNER));
            BEAST_EXPECT (channelBalance (*env.current (), chan) == chanBal);
            BEAST_EXPECT (channelAmount (*env.current (), chan) == chanAmt);
    ...
    testCancelAfter ()
    {
        ...
        auto const carol = Account ("carol");
        {
            ...
            auto const channelFunds = JBC (1000);
            env (create (
    ...
    testExpiration ()
    {
        ...
        env (fund (alice, chan, JBC (1),
            ...
            ter (temBAD_EXPIRATION));
        BEAST_EXPECT (!channelExpiration (*env.current (), chan));
        env (fund (alice, chan, JBC (1), NetClock::time_point{minExpiration}));
        env.close (minExpiration);
        // Try to extend the expiration after the expiration has already passed
        env (fund (
            alice, chan, JBC (1), NetClock::time_point{minExpiration + 1000s}));
        BEAST_EXPECT (!channelExists (*env.current (), chan));
    }
    ...
    testDisallowJBC ()
    {
        ...
        auto const bob = Account ("bob");
        {
            // Create a channel where dst disallows JBC
            Env env (*this, supported_amendments() - featureDepositAuth);
            env.fund (JBC (10000), alice, bob);
            env (fset (bob, asfDisallowJBC));
            env (create (alice, bob, JBC (1000), 3600s, alice.pk()),
                ter (tecNO_TARGET));
            auto const chan = channel (*env.current (), alice, bob);
            BEAST_EXPECT (!channelExists (*env.current (), chan));
        }
        {
            // Create a channel where dst disallows JBC.  Ignore that flag,
        ...
        {
            // Claim to a channel where dst disallows JBC
            // (channel is created before disallow jbc is set)
            Env env (*this, supported_amendments() - featureDepositAuth);
            env.fund (JBC (10000), alice, bob);
            env (create (alice, bob, JBC (1000), 3600s, alice.pk()));
            auto const chan = channel (*env.current (), alice, bob);
            ...
            env (fset (bob, asfDisallowJBC));
            auto const reqBal = JBC (500).value();
            env (claim (alice, chan, reqBal, reqBal), ter(tecNO_TARGET));
        }
        {
            // Claim to a channel where dst disallows JBC (channel is
            // created before disallow jbc is set).  Ignore that flag
    ...
    testDepositAuth ()
    {
        ...
        auto USDA = alice["USD"];
        {
            ...
            env (claim (alice, chan,
                JBC (500).value(), JBC (500).value()), ter (tecNO_PERMISSION));
            env.close();
            ...
            auto const preBob = env.balance (bob);
            {
                auto const delta = JBC (500).value();
            ...
            }
            {
                ...
                auto const delta = JBC (600).value();
                auto const sig = signClaimAuth (pk, alice.sk (), chan, delta);
            ...
            }
            {
                ...
                auto const delta = JBC (800).value();
                ...
                env.close();
                BEAST_EXPECT (
                    env.balance (bob) == preBob + JBC (800) - (5 * baseFee));
            }
        }
    }
    ...
    testMalformedPK ()
    {
        ...
        auto jv = create (alice, bob, JBC (1000), settleDelay, pk);
...
BEAST_DEFINE_TESTSUITE (PayChan, app, jbcoin);
}  // test
}  // jbcoin
