...
struct DepositAuth_test : public beast::unit_test::suite
{
    ...
    void testPayIOU()
    {
        // Exercise IOU payments and non-direct JBC payments to an account
        ...
        env (offer (carol, USD(100), JBC(100)));
        env.close();
        ...
        auto failedIouPayments = [this, &env, &alice, &bob, &USD] ()
        {
            ...
            PrettyAmount const bobJrpBalance {env.balance (bob, JBC)};
            PrettyAmount const bobUsdBalance {env.balance (bob, USD)};

            env (pay (alice, bob, USD (50)), ter (tecNO_PERMISSION));
            env.close();

            // Note that even though alice is paying bob in JBC, the payment
            ...
            BEAST_EXPECT (bobJrpBalance == env.balance (bob, JBC));
            BEAST_EXPECT (bobUsdBalance == env.balance (bob, USD));
        };

        //  Test when bob has an JBC balance > base reserve.
        failedIouPayments();

        // Set bob's JBC balance == base reserve.  Also demonstrate that
        // bob can make payments while his lsfDepositAuth flag is set.
        env (pay (bob, alice, USD(25)));
        env.close();

        {
            STAmount const bobPaysJBC {
                env.balance (bob, JBC) - reserve (env, 1)};
            JBCAmount const bobPaysFee {reserve (env, 1) - reserve (env, 0)};
            env (pay (bob, alice, bobPaysJBC), fee (bobPaysFee));
            env.close();
        }

        // Test when bob's JBC balance == base reserve.
        BEAST_EXPECT (env.balance (bob, JBC) == reserve (env, 0));
        BEAST_EXPECT (env.balance (bob, USD) == USD(25));
        failedIouPayments();

        // Test when bob has an JBC balance == 0.
        env (noop (bob), fee (reserve (env, 0)));
        env.close ();

        BEAST_EXPECT (env.balance (bob, JBC) == JBC (0));
        failedIouPayments();

        // Give bob enough JBC for the fee to clear the lsfDepositAuth flag.
    ...
    void testPayJBC()
    {
        // Exercise direct JBC payments to an account that has the
        ...
        env.close();
        BEAST_EXPECT (env.balance (bob, JBC) == JBC (10000) - drops(10));

        // bob has more JBC than the base reserve.  Any JBC payment should fail.
        env (pay (alice, bob, drops(1)), ter (tecNO_PERMISSION));
        env.close();
        BEAST_EXPECT (env.balance (bob, JBC) == JBC (10000) - drops(10));

        // Change bob's JBC balance to exactly the base reserve.
        {
            STAmount const bobPaysJBC {
                env.balance (bob, JBC) - reserve (env, 1)};
            JBCAmount const bobPaysFee {reserve (env, 1) - reserve (env, 0)};
        ...
        // bob has exactly the base reserve.  A small enough direct JBC
        ...
        BEAST_EXPECT (env.balance (bob, JBC) == reserve (env, 0) + drops(1));
        env (pay (alice, bob, drops(1)), ter (tecNO_PERMISSION));
        env.close();

        // Take bob down to a balance of 0 JBC.
        env (noop (bob), fee (reserve (env, 0) + drops(1)));
        env.close ();
        BEAST_EXPECT (env.balance (bob, JBC) == drops(0));
        ...
        env (pay (alice, bob, reserve (env, 0) + drops(0)));
        env.close();
        BEAST_EXPECT (env.balance (bob, JBC) ==
        ...
        // Take bob back down to a zero JBC balance.
        env (noop (bob), fee (env.balance (bob, JBC)));
        env.close();
        BEAST_EXPECT (env.balance (bob, JBC) == drops(0));
        ...
        env (pay (alice, bob, drops(1)));
        env.close();
        BEAST_EXPECT (env.balance (bob, JBC) == drops(1));
    ...
    void testNoJBCoin()
    {
        ...
        // any behaviors regarding rippling and the NoJBCoin flag.
        // Demonstrate that.
        testcase ("No JBCoin");
            ...
            bool withDepositAuth)
        {
            ...
            env.fund(JBC(10000), gw1, alice, bob);
            env (trust (gw1, alice["USD"](10), noJBCoinPrev ? tfSetNoJBCoin : 0));
            env (trust (gw1, bob["USD"](10), noJBCoinNext ? tfSetNoJBCoin : 0));
            ...
            TER const result = (noJBCoinNext && noJBCoinPrev)
                ? TER {tecPATH_DRY}
                : TER {tesSUCCESS};
            ...
            bool withDepositAuth)
        {
            ...
            env.fund(JBC(10000), gw1, gw2, alice);
            env (trust (alice, USD1(10), noJBCoinPrev ? tfSetNoJBCoin : 0));
            env (trust (alice, USD2(10), noJBCoinNext ? tfSetNoJBCoin : 0));
        ...
        // Test every combo of noJBCoinPrev, noJBCoinNext, and withDepositAuth
        for (int i = 0; i < 8; ++i)
        {
            auto const noJBCoinPrev = i & 0x1;
            auto const noJBCoinNext = i & 0x2;
...
struct DepositPreauth_test : public beast::unit_test::suite
{
    void testEnable()
    {
        ...
        Account const becky {"becky"};
        {
            ...
            env.fund (JBC (10000), alice, becky);
            env.close();
    ...
    void testInvalid()
    {
        ...
        }
        {
            ...
            Json::Value tx {deposit::auth (alice, becky)};
            tx[sfAuthorize.jsonName] = to_string (jbcAccount());
        ...
        // carol gets enough JBC to (barely) meet the reserve.
    ...
    void testPayment (FeatureBitset features)
    {
...

        {
            ...
            Env env (*this, features);
            env.fund (JBC (5000), alice, becky, gw);
            env.close();
            ...
            env (offer (alice, JBC (100), USD (100), tfPassive),
            ...
            env (pay (becky, becky, USD (10)),
                path (~USD), sendmax (JBC (10)), ter (expect));
            env.close();
        }
        ...
        if (supportsPreauth)
        {
            ...
            Env env (*this, features);
            env.fund (JBC (5000), alice, becky, carol, gw);
            env.close();
            ...
            // Make JBC and IOU payments from alice to becky.  Should be fine.
            env (pay (alice, becky, JBC (100)));
            ...
            env (pay (alice, becky, JBC (100)), ter (tecNO_PERMISSION));
            ...
            env (pay (becky, alice, JBC (100)), ter (tecNO_PERMISSION));
...
BEAST_DEFINE_TESTSUITE(DepositAuth,app,jbcoin);
BEAST_DEFINE_TESTSUITE(DepositPreauth,app,jbcoin);
