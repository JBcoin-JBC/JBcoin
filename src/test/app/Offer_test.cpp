...
{
    JBCAmount reserve(jtx::Env& env, std::uint32_t count)
    {
    ...
    static auto
    jbcMinusFee (jtx::Env const& env, std::int64_t jbcAmount)
        -> jtx::PrettyAmount
    {
        using namespace jtx;
        auto feeDrops = env.current()->fees().base;
        return drops (dropsPerJBC<std::int64_t>::value * jbcAmount - feeDrops);
    }
        ...
        jtx::Account const & acct_b, std::string const& currency)
    {
        ...
        jvParams[jss::jbcoin_state][jss::currency] = currency;
        jvParams[jss::jbcoin_state][jss::accounts] = Json::arrayValue;
        jvParams[jss::jbcoin_state][jss::accounts].append(acct_a.human());
        jvParams[jss::jbcoin_state][jss::accounts].append(acct_b.human());
...
public:
    void testRmFundedOffer (FeatureBitset features)
    {
        ...
        // be "taker gets" JBC. Old, broken would remove the first
        // "taker gets" jbc offer, even though the offer is still funded and
        ...
        env.fund (JBC (10000), alice, bob, carol, gw);
        ...
        // "taker gets" must be JBC
        // (Different amounts so I can distinguish the offers)
        env (offer (carol, BTC (49), JBC (49)));
        env (offer (carol, BTC (51), JBC (51)));

        // Offers for the poor quality path
        // Must be two offers at the same quality
        env (offer (carol, JBC (50), USD (50)));
        env (offer (carol, JBC (50), USD (50)));

        // Offers for the good quality path
        env (offer (carol, BTC (1), USD (100)));

        PathSet paths (Path (JBC, USD), Path (USD));
        ...
        BEAST_EXPECT(!isOffer (env, carol, BTC (1), USD (100)) &&
            isOffer (env, carol, BTC (49), JBC (49)));
    }
    ...
    void testCanceledOffer (FeatureBitset features)
    {
        ...
        env (offer (alice, JBC (500), USD (100)),
            require (offers (alice, 1)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, JBC (500), USD (100)));

        // cancel the offer above and replace it with a new offer
        auto const offer2Seq = env.seq (alice);

        env (offer (alice, JBC (300), USD (100)),
             json (jss::OfferSequence, offer1Seq),
             require (offers (alice, 1)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, JBC (300), USD (100)) &&
            !isOffer (env, alice, JBC (500), USD (100)));

        // Test canceling non-existent offer.
//      auto const offer3Seq = env.seq (alice);

        env (offer (alice, JBC (400), USD (200)),
             json (jss::OfferSequence, offer1Seq),
             require (offers (alice, 2)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, JBC (300), USD (100)) &&
            isOffer (env, alice, JBC (400), USD (200)));

        // Test cancellation now with OfferCancel tx
        auto const offer4Seq = env.seq (alice);
        env (offer (alice, JBC (222), USD (111)),
            require (offers (alice, 3)));
        env.close();

        BEAST_EXPECT(isOffer (env, alice, JBC (222), USD (111)));
        {
        ...
        env.require (offers (alice, 2));
        BEAST_EXPECT( isOffer (env, alice, JBC (300), USD (100))); // offer2
        BEAST_EXPECT(!isOffer (env, alice, JBC   (5), USD   (2))); // expired
    }
    ...
    void testJBCTinyPayment (FeatureBitset features)
    {
        testcase ("JBC Tiny payments");

        // Regression test for tiny jbc payments
        // In some cases, when the payment code calculates
        // the amount of jbc needed as input to an jbc->iou offer
        ...
        // 1) When we calculate the amount of input jbc needed for an offer
        //    from jbc->iou, the amount is less than 1 drop (after rounding
        ...
        for (auto withFix : {false, true})
        {
            ...
            env.fund (JBC (10000), alice, bob, carol, dan, erin, gw);
    ...
    void testEnforceNoJBCoin (FeatureBitset features)
    {
        testcase ("Enforce No JBCoin");
        ...
        {
            // No jbcoin with an implied account step after an offer
            Env env {*this, features};
            auto const closeTime =
                fix1449Time () +
                    100 * env.closed ()->info ().closeTimeResolution;
            ...
            env.fund (JBC (10000), alice, nojbcoin (bob), carol, dan, gw1, gw2);
            env.trust (USD1 (1000), alice, carol, dan);
            env(trust (bob, USD1 (1000), tfSetNoJBCoin));
            env.trust (USD2 (1000), alice, carol, dan);
            env(trust (bob, USD2 (1000), tfSetNoJBCoin));

            env (pay (gw1, dan, USD1 (50)));
            env (pay (gw1, bob, USD1 (50)));
            env (pay (gw2, bob, USD2 (50)));

            env (offer (dan, JBC (50), USD1 (50)));

            env (pay (alice, carol, USD2 (50)), path (~USD1, bob),
                sendmax (JBC (50)), txflags (tfNoJBCoinDirect),
                ter(tecPATH_DRY));
        }
        {
            ...
            env.fund (JBC (10000), alice, bob, carol, dan, gw1, gw2);
            ...
            env.require (balance (alice, jbcMinusFee (env, 10000 - 50)));
    ...
    testInsufficientReserve (FeatureBitset features)
    {
        ...
        // No crossing:
        {
            ...
            env (offer (alice, jbcOffer, usdOffer),
        ...
        // Partial cross:
        {
            ...
            auto const jbcOffer2 = JBC (500);

            env.fund (r + f + jbcOffer, bob);
            env (offer (bob, usdOffer2, jbcOffer2),   ter(tesSUCCESS));
            env.fund (r + f, alice);
            env (trust (alice, usdOffer),             ter(tesSUCCESS));
            env (pay (gw, alice, usdOffer),           ter(tesSUCCESS));
            env (offer (alice, jbcOffer, usdOffer),   ter(tesSUCCESS));

            env.require (
                balance (alice, r - f + jbcOffer2),
                balance (alice, usdOffer2),
                owners (alice, 1),
                balance (bob, r + jbcOffer2),
        ...
        // buy JBC. If it fully crosses, we succeed.
        {
            Env env {*this, features};
            auto const closeTime =
                fix1449Time () +
            ...
            env.fund (r + f + jbcOffer, bob, carol);
            env (offer (bob, usdOffer2, jbcOffer2),    ter(tesSUCCESS));
            env (offer (carol, usdOffer, jbcOffer),    ter(tesSUCCESS));
            ...
            env.require (
                balance (alice, r - f + jbcOffer),
                balance (alice, USD(0)),
                owners (alice, 1),
                balance (bob, r + jbcOffer2),
                balance (bob, usdOffer2),
                owners (bob, 1),
                balance (carol, r + jbcOffer2),
    ...
    testFillModes (FeatureBitset features)
    {
        ...
        auto const startBalance = JBC (1000000);
        auto const gw = Account {"gateway"};
        auto const alice = Account {"alice"};
        auto const bob = Account {"bob"};
        auto const USD = gw["USD"];
        ...
        // a fee and not place the offer on the books:
        {
            ...
            env (offer (alice, JBC (500), USD (500)),
                txflags (tfFillOrKill),              ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f + JBC (500)),
                balance (alice, USD (500)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - JBC (500)),
                balance (bob, USD (500)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // and add nothing on the books:
        {
            ...
            // Partially cross:
            env (offer (bob, USD (50), JBC (50)),            ter(tesSUCCESS));
            env (offer (alice, JBC (1000), USD (1000)),
                txflags (tfImmediateOrCancel),               ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f + JBC (50)),
                balance (alice, USD (950)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - JBC (50)),
                balance (bob, USD (50)),
                owners (bob, 1),
                offers (bob, 0));
            ...
            // Fully cross:
            env (offer (bob, USD (50), JBC (50)),            ter(tesSUCCESS));
            env (offer (alice, JBC (50), USD (50)),
                txflags (tfImmediateOrCancel),               ter(tesSUCCESS));

            env.require (
                balance (alice, startBalance - f - f - f - f + JBC (100)),
                balance (alice, USD (900)),
                owners (alice, 1),
                offers (alice, 0),
                balance (bob, startBalance - f - f - JBC (100)),
                balance (bob, USD (100)),
                owners (bob, 1),
                offers (bob, 0));
        }
        ...
        // tfPassive -- place the offer without crossing it.
        {
            ...
            env (offer (alice, USD(1000), JBC(2000)));
            env.close();
            ...
            for (auto offerPtr : aliceOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerGets] == JBC (2000));
            ...
            env (offer (bob, JBC(2000), USD(1000), tfPassive));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == JBC (2000));
            }

            // It should be possible for gw to cross both of those offers.
            env (offer (gw, JBC(2000), USD(1000)));
            env.close();
            env.require (offers (alice, 0));
            env.require (offers (gw, 0));
            env.require (offers (bob, 1));

            env (offer (gw, USD(1000), JBC(2000)));
            env.close();
        ...
        // tfPassive -- cross only offers of better quality.
        {
            ...
            env (pay (gw, "bob", USD(1000)));
            env (offer ("alice", USD(500), JBC(1001)));
            env.close();

            env (offer ("alice", USD(500), JBC(1000)));
            env.close();
            ...
            for (auto offerPtr : bobOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == JBC (999));
            }
        }
    }
    ...
    testMalformed(FeatureBitset features)
    {
        ...
        env (offer (alice, USD (1000), JBC (1000)),
        ...
        {
            // Alice tries an JBC to JBC order:
            env (offer (alice, JBC (1000), JBC (1000)),   ter(temBAD_OFFER));
            env.require (
                owners (alice, 0),
                offers (alice, 0));
        ...
        {
            env (offer (alice, -USD (1000), JBC (1000)),  ter(temBAD_OFFER));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        ...
        // Use JBC as a currency code
        {
            auto const BAD = IOU(gw, badCurrency());

            env (offer (alice, JBC (1000), BAD (1000)),   ter(temBAD_CURRENCY));
            env.require (
                owners (alice, 1),
                offers (alice, 0));
        }
    ...
    testExpiration(FeatureBitset features)
    {
        ...
        env (offer (bob, usdOffer, jbcOffer),     ter(tesSUCCESS));
        env.require (
            balance (alice, startBalance - f - f - f),
            balance (alice, usdOffer),
            offers (alice, 0),
    ...
    testUnfundedCross(FeatureBitset features)
    {
        ...
        env (offer ("alice", usdOffer, jbcOffer),     ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env (offer ("bob", usdOffer, jbcOffer),       ter(tecUNFUNDED_OFFER));
        env.require (
        ...
        env.fund (reserve (env, 0) + f + JBC(1), "carol");
        env (offer ("carol", usdOffer, jbcOffer), ter(tecINSUF_RESERVE_OFFER));
        env.require (
            balance ("carol", reserve (env, 0) + JBC(1)),
            owners ("carol", 0));

        // Account has enough for the reserve plus one
        // offer, and the fee.
        env.fund (reserve (env, 1) + f, "dan");
        env (offer ("dan", usdOffer, jbcOffer),       ter(tesSUCCESS));
        env.require (
        ...
        env.fund (reserve (env, 1) + f + jbcOffer, "eve");
        env (offer ("eve", usdOffer, jbcOffer),       ter(tesSUCCESS));
        env.require (
            balance ("eve", reserve (env, 1) + jbcOffer),
            owners ("eve", 1));
    }
    ...
    testSelfCross(bool use_partner, FeatureBitset features)
    {
        ...
        env.fund (JBC (10000), gw);
        if (use_partner)
        {
            env.fund (JBC (10000), partner);
        ...
        // through JBC
        env (offer (account_to_test, BTC (250), JBC (1000)),
                 offers (account_to_test, 1));

        // validate that the book now shows a BTC for JBC offer
        BEAST_EXPECT(isOffer(env, account_to_test, BTC(250), JBC(1000)));

        auto const secondLegSeq = env.seq(account_to_test);
        env (offer (account_to_test, JBC(1000), USD (50)),
                 offers (account_to_test, 2));

        // validate that the book also shows a JBC for USD offer
        BEAST_EXPECT(isOffer(env, account_to_test, JBC(1000), USD(50)));
        ...
        // behavior.
        {
            ...
            for (auto const& offerPtr : acctOffers)
            {
                ...
                BEAST_EXPECT (offer[sfTakerPays] == JBC (0));
            }
        }
    ...
    testNegativeBalance(FeatureBitset features)
    {
        ...
        env (offer (alice, USD (50), JBC (150000)));
        ...
        // small_amount and transfer no JBC.  The new offer crossing transfers
        ...
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "50");
        BEAST_EXPECT(env.balance (alice, jbcIssue()) ==
        ...
        BEAST_EXPECT(env.balance (bob, jbcIssue()) ==
            bob_initial_balance -
    ...
    void
    testOfferCrossWithJBC(bool reverse_order, FeatureBitset features)
    {
        testcase (std::string("Offer Crossing with JBC, ") +
        ...
        env.fund (JBC (10000), gw, alice, bob);
        ...
        if (reverse_order)
            env (offer (bob, USD (1), JBC (4000)));

        env (offer (alice, JBC (150000), USD (50)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                JBC (10000).value ().mantissa () -
                JBC (reverse_order ? 4000 : 3000).value ().mantissa () -
    ...
    testOfferCrossWithLimitOverride(FeatureBitset features)
    {
        ...
        env.fund (JBC (100000), gw, alice, bob);
        ...
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                JBC (100000).value ().mantissa () -
                JBC (3000).value ().mantissa () -
    ...
    testOfferAcceptThenCancel(FeatureBitset features)
    {
        ...
        env (offer (env.master, JBC (500), USD (100)));
        env.close ();
    ...
    testCurrencyConversionEntire(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == JBC (500).value ().getText ());
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == USD (100).value ().getJson (0));

        env (pay (alice, alice, JBC (500)), sendmax (USD (100)));
    ...
    testCurrencyConversionIntoDebt(FeatureBitset features)
    {
        ...
        env.fund (JBC (10000), alice, bob, carol);
    ...
    testCurrencyConversionInParts(FeatureBitset features)
    {
        ...
        env (pay (alice, alice, JBC (200)), sendmax (USD (100)));

        // The previous payment reduced the remaining offer amount by 200 JBC
        auto jro = ledgerEntryOffer (env, bob, bobOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == JBC (300).value ().getText ());
        BEAST_EXPECT(
        ...
        // alice now has 200 more JBC from the payment
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] ==
            std::to_string(
                JBC (10000).value ().mantissa () +
                JBC (200).value ().mantissa () -
                env.current ()->fees ().base * 2)
        );
        ...
        // Alice converts USD to JBC which should fail
        // due to PartialPayment.
        env (pay (alice, alice, JBC (600)), sendmax (USD (100)),
            ter(tecPATH_PARTIAL));

        // Alice converts USD to JBC, should succeed because
        ...
        // only 300 JBC should be have been payed since that's all
    ...
    void
    testCrossCurrencyStartJBC(FeatureBitset features)
    {
        testcase ("Cross Currency Payment: Start with JBC");
        ...
        env.fund (JBC (10000), gw, alice, bob, carol);
        ...
        env (offer (carol, JBC (500), USD (50)));

        env (pay (alice, bob, USD (25)), sendmax (JBC (333)));
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == JBC (250).value ().getText ());
    }

    void
    testCrossCurrencyEndJBC(FeatureBitset features)
    {
        testcase ("Cross Currency Payment: End with JBC");
        ...
        auto const carolOfferSeq = env.seq (carol);
        env (offer (carol, USD (50), JBC (500)));

        env (pay (alice, bob, JBC (250)), sendmax (USD (333)));
        ...
        BEAST_EXPECT(
            ...
            std::to_string(
                JBC (10000).value ().mantissa () +
                JBC (250).value ().mantissa ())
        );

        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == JBC (250).value ().getText ());
        BEAST_EXPECT(
    ...
    testCrossCurrencyBridged(FeatureBitset features)
    {
        ...
        env.fund (JBC (10000), gw1, gw2, alice, bob, carol, dan);
        ...
        env (offer (dan, JBC (500), EUR (50)));

        Json::Value jtp {Json::arrayValue};
        jtp[0u][0u][jss::currency] = "JBC";
        ...
        auto jro = ledgerEntryOffer (env, carol, carolOfferSeq);
        BEAST_EXPECT(
            jro[jss::node][jss::TakerGets] == JBC (200).value ().getText ());
        BEAST_EXPECT(
        ...
        BEAST_EXPECT(
            jro[jss::node][jss::TakerPays] == JBC (200).value ().getText ());
    }
    ...
    testOfferFeesConsumeFunds(FeatureBitset features)
    {
        ...
        //  1 for payment          == 4
        auto const starting_jbc = JBC (100) +
            env.current ()->fees ().accountReserve (3) +
            env.current ()->fees ().base * 4;

        env.fund (starting_jbc, gw1, gw2, gw3, alice, bob);
        ...
        env (offer (bob, JBC (200), USD1 (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD1 (200), JBC (200)));

        auto jrr = ledgerEntryState (env, alice, gw1, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == JBC (350).value ().getText ()
        );
    ...
    testSellFlagBasic(FeatureBitset features)
    {
        ...
        env.fund (starting_jbc, gw, alice, bob);
        ...
        env (offer (bob, JBC (200), USD (200)), json(jss::Flags, tfSell));
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Alice has 350 + fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        env (offer (alice, USD (200), JBC (200)), json(jss::Flags, tfSell));

        auto jrr = ledgerEntryState (env, alice, gw, "USD");
        BEAST_EXPECT(jrr[jss::node][sfBalance.fieldName][jss::value] == "-100");
        jrr = ledgerEntryRoot (env, alice);
        BEAST_EXPECT(
            jrr[jss::node][sfBalance.fieldName] == JBC (250).value ().getText ()
        );
    ...
    testSellFlagExceedLimit(FeatureBitset features)
    {
        ...
        env (offer (bob, JBC (100), USD (200)));
        // Alice has 350 fees - a reserve of 50 = 250 reserve = 100 available.
        // Ask for more than available to prove reserve works.
        // Taker pays 100 USD for 100 JBC.
        // Selling JBC.
        // Will sell all 100 JBC and get more USD than asked for.
        env (offer (alice, USD (100), JBC (100)), json(jss::Flags, tfSell));
    ...
    testGatewayCrossCurrency(FeatureBitset features)
    {
        ...
        auto const starting_jbc = JBC (100.1) +
            env.current ()->fees ().accountReserve (1) +
            env.current ()->fees ().base * 2;
        ...
        if (wsc->version() == 2)
        {
            ...
            BEAST_EXPECT(jrr.isMember(jss::jbcoinrpc) && jrr[jss::jbcoinrpc] == "2.0");
    ...
    void testPartialCross (FeatureBitset features)
    {
        ...
        env.fund (JBC(10000000), gw);

        // The fee that's charged for transactions
        auto const f = env.current ()->fees ().base;

        // To keep things simple all offers are 1 : 1 for JBC : USD.
        enum preTrustType {noPreTrust, gwPreTrust, acctPreTrust};
        struct TestData
        {
            std::string account;       // Account operated on
            STAmount fundJrp;          // Account funded with
            int bookAmount;            // USD -> JBC offer on the books
            preTrustType preTrust;     // If true, pre-establish trust line
            int offerAmount;           // Account offers this much JBC -> USD
            TER tec;                   // Returned tec code
            STAmount spentJrp;         // Amount removed from fundJrp
        ...
        {
//acct                             fundJrp  bookAmt      preTrust  offerAmt                     tec       spentJrp    balanceUSD  offers  owners
...
{"flo",             reserve (env, 1) + 0*f,       1,   noPreTrust,     1000,             tesSUCCESS, JBC(   1) + f, USD(      1),      0,     1},
{"gay",             reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, JBC(  50) + f, USD(     50),      0,     1},
{"hye", JBC(1000)                    + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, JBC( 800) + f, USD(    800),      0,     1},
{"ivy", JBC(   1) + reserve (env, 1) + 1*f,       1,   noPreTrust,     1000,             tesSUCCESS, JBC(   1) + f, USD(      1),      0,     1},
{"joy", JBC(   1) + reserve (env, 2) + 1*f,       1,   noPreTrust,     1000,             tesSUCCESS, JBC(   1) + f, USD(      1),      1,     2},
{"kim", JBC( 900) + reserve (env, 2) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, JBC( 999) + f, USD(    999),      0,     1},
{"liz", JBC( 998) + reserve (env, 0) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, JBC( 998) + f, USD(    998),      0,     1},
{"meg", JBC( 998) + reserve (env, 1) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, JBC( 999) + f, USD(    999),      0,     1},
{"nia", JBC( 998) + reserve (env, 2) + 1*f,     999,   noPreTrust,     1000,             tesSUCCESS, JBC( 999) + f, USD(    999),      1,     2},
{"ova", JBC( 999) + reserve (env, 0) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, JBC( 999) + f, USD(    999),      0,     1},
{"pam", JBC( 999) + reserve (env, 1) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, JBC(1000) + f, USD(   1000),      0,     1},
{"rae", JBC( 999) + reserve (env, 2) + 1*f,    1000,   noPreTrust,     1000,             tesSUCCESS, JBC(1000) + f, USD(   1000),      0,     1},
{"sue", JBC(1000) + reserve (env, 2) + 1*f,       0,   noPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
...
{"eli", JBC(  20) + reserve (env, 0) + 1*f,    1000,   gwPreTrust,     1000,             tesSUCCESS, JBC(20) + 1*f, USD(     20),      0,     0},
{"fyn",             reserve (env, 1) + 0*f,       0,   gwPreTrust,     1000,             tesSUCCESS,             f, USD(      0),      1,     1},
{"gar",             reserve (env, 1) + 0*f,       1,   gwPreTrust,     1000,             tesSUCCESS, JBC(   1) + f, USD(      1),      1,     1},
{"hal",             reserve (env, 1) + 1*f,       1,   gwPreTrust,     1000,             tesSUCCESS, JBC(   1) + f, USD(      1),      1,     1},
...
{"ted", JBC(  20) + reserve (env, 1) + 2*f,    1000, acctPreTrust,     1000,             tesSUCCESS, JBC(20) + 2*f, USD(     20),      0,     1},
{"uli",             reserve (env, 2) + 0*f,       0, acctPreTrust,     1000, tecINSUF_RESERVE_OFFER,           2*f, USD(      0),      0,     1},
{"vic",             reserve (env, 2) + 0*f,       1, acctPreTrust,     1000,             tesSUCCESS, JBC( 1) + 2*f, USD(      1),      0,     1},
{"wes",             reserve (env, 2) + 1*f,       0, acctPreTrust,     1000,             tesSUCCESS,           2*f, USD(      0),      1,     2},
{"xan",             reserve (env, 2) + 1*f,       1, acctPreTrust,     1000,             tesSUCCESS, JBC( 1) + 2*f, USD(      1),      1,     2},
};

        for (auto const& t : tests)
        {
            auto const acct = Account(t.account);
            env.fund (t.fundJrp, acct);
            env.close();
            ...
            if (book)
                env (offer (gw, JBC (book), USD (book)));
            env.close();
            ...
            // to be enough JBC reserve for acct to create the trust line.
            ...
            env (offer (acct, USD (acctOffer), JBC (acctOffer)), ter (t.tec));
            env.close();
            std::uint32_t const acctOfferSeq = env.seq (acct) - 1;

            BEAST_EXPECT (env.balance (acct, USD.issue()) == t.balanceUsd);
            BEAST_EXPECT (
                env.balance (acct, jbcIssue()) == t.fundJrp - t.spentJrp);
            ...
            if (acctOffers.size() && t.offers)
            {
                ...
                BEAST_EXPECT (acctOffer[sfTakerGets] == JBC (leftover));
    ...
    void
    testJBCDirectCross (FeatureBitset features)
    {
        testcase ("JBC Direct Crossing");
        ...
        // The scenario:
        //   o alice has USD but wants JBC.
        //   o bob has JBC but wants USD.
        auto const alicesJBC = env.balance (alice);
        auto const bobsJBC = env.balance (bob);

        env (offer (alice, jbcOffer, usdOffer));
        env.close();
        env (offer (bob, usdOffer, jbcOffer));

        env.close();
        env.require (
            balance (alice, USD(0)),
            balance (bob, usdOffer),
            balance (alice, alicesJBC + jbcOffer - fee),
            balance (bob,   bobsJBC   - jbcOffer - fee),
            offers (alice, 0),
            offers (bob, 0));
        ...
        env (offer (alice, USD(999), JBC(999)));
        env (offer (bob, jbcOffer, usdOffer));
        ...
        verifyDefaultTrustline (env, bob, USD(1));
        {
            ...
            BEAST_EXPECT (bobsOffer[sfTakerPays] == JBC (1));
        }
   }
    ...
    testBridgedCross (FeatureBitset features)
    {
        ...
        env.fund (JBC(1000000), gw, alice, bob, carol);
        env.close();
        ...
        env (offer (alice, JBC(1000), usdOffer));
        env (offer (bob, eurOffer, JBC(1000)));
        auto const bobJrpBalance = env.balance (bob);
        env.close();
        ...
        env.require (
            ...
            balance (bob, bobJrpBalance - JBC(400)),
            offers (carol, 0));
        ...
        verifyDefaultTrustline (env, carol, USD(400));
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == JBC (600));
        }
        {
            auto const bobsOffers = offersOnAccount (env, bob);
            BEAST_EXPECT (bobsOffers.size() == 1);
            ...
            BEAST_EXPECT (bobsOffer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (bobsOffer[sfTakerGets] == JBC (600));
        ...
        env.require (
            ...
            balance (bob, bobJrpBalance - JBC(1000)),
            offers (bob, 0),
            offers (carol, 0));
        ...
        if (alicesOffers.size() != 0)
        {
            ...
            BEAST_EXPECT (alicesOffer[sfTakerPays] == JBC (0));
        }
    }
    ...
    testSellOffer (FeatureBitset features)
    {
        ...
        {
            std::string account;       // Account operated on
            STAmount fundJrp;          // JBC acct funded with
            ...
            TestData (
                ...
                STAmount const& fundJrp_,       // JBC acct funded with
                ...
                STAmount const& spentJrp_,      // Amount removed from fundJrp
            ...
            TestData (
                ...
                int offers_,                    // Offers on acct
                int owners_)                    // Owners on acct
                : TestData (std::move(account_), fundJrp_, fundUSD_, gwGets_,
                  gwPays_, acctGets_, acctPays_, tec_, spentJrp_, finalUsd_,
        ...
        {
// acct pays JBC
//acct                           fundJrp  fundUSD   gwGets   gwPays  acctGets  acctPays                      tec         spentJrp  finalUSD  offers  owners  takerGets  takerPays
{"ann", JBC(10) + reserve (env, 0) + 1*f, USD( 0), JBC(10), USD( 5),  USD(10),  JBC(10), tecINSUF_RESERVE_OFFER, JBC( 0) + (1*f),  USD( 0),      0,     0},
{"bev", JBC(10) + reserve (env, 1) + 1*f, USD( 0), JBC(10), USD( 5),  USD(10),  JBC(10),             tesSUCCESS, JBC( 0) + (1*f),  USD( 0),      1,     1,     JBC(10),  USD(10)},
{"cam", JBC(10) + reserve (env, 0) + 1*f, USD( 0), JBC(10), USD(10),  USD(10),  JBC(10),             tesSUCCESS, JBC(10) + (1*f),  USD(10),      0,     1},
{"deb", JBC(10) + reserve (env, 0) + 1*f, USD( 0), JBC(10), USD(20),  USD(10),  JBC(10),             tesSUCCESS, JBC(10) + (1*f),  USD(20),      0,     1},
{"eve", JBC(10) + reserve (env, 0) + 1*f, USD( 0), JBC(10), USD(20),  USD( 5),  JBC( 5),             tesSUCCESS, JBC( 5) + (1*f),  USD(10),      0,     1},
{"flo", JBC(10) + reserve (env, 0) + 1*f, USD( 0), JBC(10), USD(20),  USD(20),  JBC(20),             tesSUCCESS, JBC(10) + (1*f),  USD(20),      0,     1},
{"gay", JBC(20) + reserve (env, 1) + 1*f, USD( 0), JBC(10), USD(20),  USD(20),  JBC(20),             tesSUCCESS, JBC(10) + (1*f),  USD(20),      0,     1},
{"hye", JBC(20) + reserve (env, 2) + 1*f, USD( 0), JBC(10), USD(20),  USD(20),  JBC(20),             tesSUCCESS, JBC(10) + (1*f),  USD(20),      1,     2,     JBC(10),  USD(10)},
// acct pays USD
{"meg",           reserve (env, 1) + 2*f, USD(10), USD(10), JBC( 5),  JBC(10),  USD(10), tecINSUF_RESERVE_OFFER, JBC(  0) + (2*f),  USD(10),      0,     1},
{"nia",           reserve (env, 2) + 2*f, USD(10), USD(10), JBC( 5),  JBC(10),  USD(10),             tesSUCCESS, JBC(  0) + (2*f),  USD(10),      1,     2,     USD(10),  JBC(10)},
{"ova",           reserve (env, 1) + 2*f, USD(10), USD(10), JBC(10),  JBC(10),  USD(10),             tesSUCCESS, JBC(-10) + (2*f),  USD( 0),      0,     1},
{"pam",           reserve (env, 1) + 2*f, USD(10), USD(10), JBC(20),  JBC(10),  USD(10),             tesSUCCESS, JBC(-20) + (2*f),  USD( 0),      0,     1},
{"qui",           reserve (env, 1) + 2*f, USD(10), USD(20), JBC(40),  JBC(10),  USD(10),             tesSUCCESS, JBC(-20) + (2*f),  USD( 0),      0,     1},
{"rae",           reserve (env, 2) + 2*f, USD(10), USD( 5), JBC( 5),  JBC(10),  USD(10),             tesSUCCESS, JBC( -5) + (2*f),  USD( 5),      1,     2,     USD( 5),  JBC( 5)},
{"sue",           reserve (env, 2) + 2*f, USD(10), USD( 5), JBC(10),  JBC(10),  USD(10),             tesSUCCESS, JBC(-10) + (2*f),  USD( 5),      1,     2,     USD( 5),  JBC( 5)},
};
    ...
    testSellWithFillOrKill (FeatureBitset features)
    {
        ...
        env.fund (JBC(10000000), gw, alice, bob);

        // bob offers JBC for USD.
        env (trust(bob, USD(200)));
        env.close();
        env (pay(gw, bob, USD(100)));
        env.close();
        env (offer(bob, JBC(2000), USD(20)));
        env.close();
        {
            ...
            env (offer(alice, USD(21), JBC(2100), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            env (offer(alice, USD(20), JBC(2000), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            env (offer(bob, JBC(2000), USD(20)));
            env.close();
            env (offer(alice, USD(10), JBC(1500), tfSell | tfFillOrKill));
            env.close();
        ...
        }
        {
            ...
            // We're using bob's left-over offer for JBC(500), USD(5)
            env (offer(alice, USD(1), JBC(501), tfSell | tfFillOrKill));
            env.close();
            env.require (balance (alice, USD(35)));
            env.require (offers (alice, 0));
            env.require (balance (bob, USD(65)));
        }
        {
            ...
            // We're using bob's left-over offer for JBC(500), USD(5)
            env (offer(alice, USD(1), JBC(500), tfSell | tfFillOrKill));
            env.close();
    ...
    testTransferRateOffer (FeatureBitset features)
    {
        ...
        env(rate(gw1, 1.25));
        {
            ...
            auto const bob = Account("bob");
            env.fund (JBC(100) + reserve(env, 2) + (fee*2), ann, bob);
            env.close();
            ...
            // bob offers to sell USD(100) for JBC.  alice takes bob's offer.
            ...
            env (offer (bob, JBC(1), USD(100)));
            env.close();

            env (offer (ann, USD(100), JBC(1)));
            env.close();

            env.require (balance (ann, USD(100)));
            env.require (balance (ann, JBC( 99) + reserve(env, 2)));
            env.require (offers (ann, 0));

            env.require (balance (bob, USD(  0)));
            env.require (balance (bob, JBC(101) + reserve(env, 2)));
            env.require (offers (bob, 0));
        }
        {
            // Reverse the order, so the offer in the books is to sell JBC
            // in return for USD.  Gateway rate should still apply identically.
            auto const che = Account("che");
            auto const deb = Account("deb");
            env.fund (JBC(100) + reserve(env, 2) + (fee*2), che, deb);
            env.close();
            ...
            env (offer (che, USD(100), JBC(1)));
            env.close();

            env (offer (deb, JBC(1), USD(100)));
            env.close();

            env.require (balance (che, USD(100)));
            env.require (balance (che, JBC( 99) + reserve(env, 2)));
            env.require (offers (che, 0));

            env.require (balance (deb, USD(  0)));
            env.require (balance (deb, JBC(101) + reserve(env, 2)));
        ...
        }
        {
            ...
            env.fund (JBC(20000) + fee*2, eve, fyn);
            env.close();
            ...
            env (offer (eve, USD(10), JBC(4000)));
            env.close();
            std::uint32_t const eveOfferSeq = env.seq (eve) - 1;

            env (offer (fyn, JBC(2000), USD(5)));
            env.close();

            env.require (balance (eve, USD(105)));
            env.require (balance (eve, JBC(18000)));
            ...
            if (evesOffers.size() != 0)
            {
                ...
                BEAST_EXPECT (evesOffer[sfTakerGets] == JBC (2000));
            ...
            env.require (balance (fyn, JBC(22000)));
        ...
        env.fund (JBC(100000), gw2);
        env.close();

        env(rate(gw2, 1.5));
        {
            // Remove JBC from the equation.  Give the two currencies two
        ...
        }
        {
            ...
            env.fund (JBC(2) + reserve(env, 3) + (fee*3), ova, pat, qae);
            env.close();

            //   o ova has USD but wants XPR.
            //   o pat has JBC but wants EUR.
            ...
            env (offer (ova, JBC(2), USD(100)));
            env (offer (pat, EUR(100), JBC(2)));
            env.close();
            ...
            env.require (balance (ova, JBC(4) + reserve(env, 3)));
    ...
    testSelfCrossOffer1 (FeatureBitset features)
    {
        ...
        for (auto const& offerPtr : gwOffers)
        {
            auto const& offer = *offerPtr;
            BEAST_EXPECT (offer[sfLedgerEntryType] == ltOFFER);
            BEAST_EXPECT (offer[sfTakerGets] == JBC (600));
        ...
        for (auto const& offerPtr : gwOffers)
        {
            ...
            BEAST_EXPECT (offer[sfTakerPays] == JBC (1000));
        }
    }
    ...
    testSelfCrossOffer2 (FeatureBitset features)
    {
        ...
        struct TestData
        {
            ...
            STAmount fundJBC;          // JBC acct funded with
            STAmount fundUSD;          // USD acct funded with
        ...
        TestData const tests[]
        {
// acct               fundJBC    fundUSD    fundEUR           firstOfferTec          secondOfferTec
...
{"flo", reserve(env, 3) +   0, USD(1000), EUR(1000), tecINSUF_RESERVE_OFFER, tecINSUF_RESERVE_OFFER},
    ...
    testSelfIssueOffer (FeatureBitset features)
    {
        ...
        env.fund(JBC(50000) + f, alice, bob);
        env.close();

        env(offer(alice, USD(5000), JBC(50000)));
        env.close();

        // This offer should take alice's offer up to Alice's reserve.
        env(offer(bob, JBC(50000), USD(5000)));
        env.close();

        // alice's offer should have been removed, since she's down to her
        // JBC reserve.
        env.require (balance (alice, JBC(250)));
        ...
        for (auto const& offerPtr : bobOffers)
        {
            ...
            BEAST_EXPECT(offer[sfTakerPays] == JBC (250));
        }
    }
    ...
    void testSelfCrossLowQualityOffer (FeatureBitset features)
    {
        ...
        // for more JBC than the offering account held.  This unit test
    ...
    void testTinyOffer (FeatureBitset features)
    {
        ...
        auto const CNY = gw["CNY"];
        auto const fee = env.current()->fees().base;
        auto const startJrpBalance = drops (400000000000) + (fee * 2);

        env.fund (startJrpBalance, gw, alice, bob);
        env.close();
        ...
        env.require (balance (alice, startJrpBalance - fee - drops(1)));
        env.require (balance (bob, bobsCnyStartBalance - alicesCnyOffer));
        env.require (balance (bob, startJrpBalance - (fee * 2) + drops(1)));
    }
    ...
    void testSelfPayXferFeeOffer (FeatureBitset features)
    {
        ...
        //
        //  2. alice makes an offer to buy JBC and sell USD.
        //  3. bob makes an offer to buy BTC and sell JBC.
        //
        ...
        //  o bob' offer which takes alice's BTC and sells JBC to...
        //  o alice's first offer which takes bob's JBC and sells USD to...
        ...
        auto const BTC = gw["BTC"];
        auto const USD = gw["USD"];
        auto const startJrpBalance = JBC (4000000);

        env.fund (startJrpBalance, gw);
        env.close();
        ...
        struct Actor
        {
            ...
            PrettyAmount jbc;  // final expected after crossing
...
// { 0, 0, 0, BTC( 5), { {"flo", 0, drops(3999999999950), BTC( 5.0), USD(2000)}                                                      } }  // no xfer fee
        ...
        for (auto const& t : tests)
        {
            ...
            {
                env.fund (JBC (4000000), actor.acct);
                env.close();
            ...
            env (offer (leg0, BTC(10),     JBC(100000), tfPassive));
            env.close();
            std::uint32_t const leg0OfferSeq = env.seq (leg0) - 1;

            env (offer (leg1, JBC(100000), USD(1000),   tfPassive));
            env.close();
            ...
            for (auto const& actor : t.actors)
            {
                ...
                env.require (balance (actor.acct, actor.jbc));
...
{ 0, 0, 0, BTC( 5), { {"hye", 2, drops(3999999999950), BTC(5), USD(2000)}                                                      } }  // no xfer fee
    ...
    void testRequireAuth (FeatureBitset features)
    {
        ...
        env.fund (JBC(400000), gw, alice, bob);
        env.close();
        ...
        env (offer (alice, gwUSD(40), JBC(4000)));
        env.close();
        ...
        env (offer (bob, JBC(4000), gwUSD(40)));
        env.close();
    ...
    void testMissingAuth (FeatureBitset features)
    {
        ...
        // untouched.
        env (offer (alice, gwUSD(40), JBC(4000)), ter(tecNO_LINE));
        env.close();
        ...
        env (offer (alice, gwUSD(40), JBC(4000)), ter(tecNO_AUTH));
        env.close();
    ...
    {
        testcase("JBCoinConnect Smoketest payment flow");
        using namespace jtx;
        ...
        // This test mimics the payment flow used in the JBCoin Connect
        ...
        env.fund (JBC(100000), hotUS, coldUS, hotEU, coldEU, mm);
        env.close();

        // Cold wallets require trust but will jbcoin by default
        for (auto const& cold : {coldUS, coldEU})
        {
            env(fset (cold, asfRequireAuth));
            env(fset (cold, asfDefaultJBCoin));
        }
        env.close();

        // Each hot wallet trusts the related cold wallet for a large amount
        env (trust(hotUS, USD(10000000)), txflags (tfSetNoJBCoin));
        env (trust(hotEU, EUR(10000000)), txflags (tfSetNoJBCoin));
        // Market maker trusts both cold wallets for a large amount
        env (trust(mm, USD(10000000)), txflags (tfSetNoJBCoin));
        env (trust(mm, EUR(10000000)), txflags (tfSetNoJBCoin));
        env.close();
        ...
        // There should be a path available from hot US to cold EUR
        {
            ...
            Json::Value const jrr {env.rpc(
                "json", "jbcoin_path_find", to_string(jvParams))[jss::result]};
    ...
    void testSelfAuth (FeatureBitset features)
    {
        ...
        env.fund (JBC(400000), gw, alice);
        env.close();

        // Test that gw can create an offer to buy gw's currency.
        env (offer (gw, gwUSD(40), JBC(4000)));
        env.close();
        std::uint32_t const gwOfferSeq = env.seq (gw) - 1;
        env.require (offers (gw, 1));
        ...
        env (offer (alice, JBC(4000), gwUSD(40)));
        env.close();
    ...
    void testAll(FeatureBitset features)
    {
        ...
        testJBCTinyPayment(features);
        testEnforceNoJBCoin(features);
        ...
        testOfferCrossWithJBC(true, features);
        testOfferCrossWithJBC(false, features);
        ...
        testCrossCurrencyStartJBC(features);
        testCrossCurrencyEndJBC(features);
...
BEAST_DEFINE_TESTSUITE_PRIO (Offer, tx, jbcoin, 4);
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO (Offer_manual, tx, jbcoin, 20);
