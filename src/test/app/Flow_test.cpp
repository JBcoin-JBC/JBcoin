...
bool getNoJBCoinFlag (jtx::Env const& env,
    ...
    Currency const& cur)
{
    ...
    {
        auto const flag = (src.id() > dst.id()) ? lsfHighNoJBCoin : lsfLowNoJBCoin;
...
jbcMinusFee (jtx::Env const& env, std::int64_t jbcAmount)
{
    ...
    return drops (
        dropsPerJBC<std::int64_t>::value * jbcAmount - feeDrops);
};
...
struct Flow_test : public beast::unit_test::suite
{
    void testDirectStep (FeatureBitset features)
    {
        ...
        auto const USD = gw["USD"];
        {
            ...
            env.fund (JBC (10000), alice, bob, gw);
        ...
        }
        {
            ...
            env (pay (alice, bob, JBC (100)));
            env.require (balance (bob, JBC (10000 + 100)));
            env.require (balance (alice, jbcMinusFee (env, 10000 - 100)));
        }
        {
            // Partial payments
            Env env (*this, features);
        ...
        }
        {
            ...
            env.fund (JBC (10000), alice, bob, carol, dan);
        ...
        }
        {
            ...
            env (pay (alice, dan, USDC (5)), path (bob, carol),
                sendmax (USDA (6)), txflags (tfNoJBCoinDirect));
        ...
        }
        {
            ...
            env.fund (JBC (10000), alice, bob, carol, dan, erin);
            ...
            env (pay (alice, erin, USDD (5)), path (carol, dan),
                path (bob, dan), txflags (tfNoJBCoinDirect));
        ...
        for (auto bobDanQIn : {80, 100, 120})
            for (auto bobAliceQOut : {80, 100, 120})
            {
                ...
                env(pay(dan, carol, USDA(10)),
                    path(bob), sendmax(USDD(100)), txflags(tfNoJBCoinDirect));
    ...
    void testBookStep (FeatureBitset features)
    {
        ...
        }
        {
            ...
            env (offer (bob, BTC (50), JBC (50)));
            env (offer (bob, JBC (50), USD (50)));

            env (pay (alice, carol, USD (50)), path (~JBC, ~USD),
                sendmax (BTC (50)));
            ...
            env.require (balance (bob, USD (0)));
            env.require (balance (carol, USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, JBC (50), USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, BTC (50), JBC (50)));
        }
        {
            // simple JBC -> USD through offer and sendmax
            Env env (*this, features);
            ...
            env.require (balance (bob, jbcMinusFee (env, 10000 + 50)));
            env.require (balance (bob, USD (0)));
            env.require (balance (carol, USD (50)));
            BEAST_EXPECT(!isOffer (env, bob, JBC (50), USD (50)));
        }
        {
            // simple USD -> JBC through offer and sendmax
            Env env (*this, features);
            ...
            env (offer (bob, USD (50), JBC (50)));

            env (pay (alice, carol, JBC (50)), path (~JBC),
                 sendmax (USD (50)));

            env.require (balance (alice, USD (0)));
            env.require (balance (bob, jbcMinusFee (env, 10000 - 50)));
            env.require (balance (bob, USD (50)));
            env.require (balance (carol, JBC (10000 + 50)));
            BEAST_EXPECT(!isOffer (env, bob, USD (50), JBC (50)));
        }
        {
        ...
        }
        {
            ...
            env (pay (alice, carol, EUR (1)), path (~JBC, ~EUR),
                sendmax (USD (0.4)), txflags (tfNoJBCoinDirect|tfPartialPayment));
    ...
    void testTransferRate (FeatureBitset features)
    {
        ...
        }
        {
            ...
            env (pay (alice, carol, USD (50)), path (~USD), sendmax (JBC (50)));
            env.require (
                balance (alice, jbcMinusFee (env, 10000 - 50)),
...

        {
            ...
            env (pay (alice, carol, EUR (40)), path (~USD, ~EUR), sendmax (JBC (40)));
            env.require (
                balance (alice,  jbcMinusFee (env, 10000 - 40)),
                balance (bob, USD (40)),
                balance (bob, EUR (0)),
        ...
        }
        {
            ...
            env.fund (JBC (10000), alice, bob, carol, dan, gw);
            env(rate(gw, 1.25));
            ...
            env (pay (alice, carol, EUR (50)), path (bob, gw, ~EUR),
                sendmax (USDA (60)), txflags (tfNoJBCoinDirect));
            env.require (
                balance (bob, USD (-10)),
                balance (bob, USDA (60)),
                balance (dan, USD (50)),
...

        {
            ...
            env(rate(gw, 1.25));
            env.trust (USD (1000), alice, bob);
            env (offer (gw, JBC (100), USD (100)));
            env (pay (alice, bob, USD (100)),
                 sendmax (JBC (100)));
            env.require (
                balance (alice, jbcMinusFee(env, 10000-100)),
                balance (bob, USD (100)));
        }
        if (!features[featureOwnerPaysFee])
        {
            ...
            env(rate(gw, 1.25));
            env.trust (USD (1000), alice, bob);
            env (offer (gw, JBC (125), USD (125)));
            env (pay (alice, bob, USD (100)),
                 sendmax (JBC (200)));
            env.require (
                balance (alice, jbcMinusFee(env, 10000-125)),
                balance (bob, USD (100)));
        }
    }
    ...
    testFalseDry(FeatureBitset features)
    {
        ...
        env.fund (JBC(10000), alice, carol, gw);
        ...
        // Bob has _just_ slightly less than 50 jbc available
        // If his owner count changes, he will have more liquidity.
        // This is one error case to test (when Flow is used).
        // Computing the incomming jbc to the JBC/USD offer will require two
        // recursive calls to the EUR/JBC offer. The second call will return
        // tecPATH_DRY, but the entire path should not be marked as dry. This
        // is the second error case to test (when flowV1 is used).
        env (offer (bob, EUR (50), JBC (50)));
        env (offer (bob, JBC (50), USD (50)));

        env (pay (alice, carol, USD (1000000)), path (~JBC, ~USD),
            sendmax (EUR (500)),
    ...
    testLimitQuality ()
    {
        ...
        for (auto const& d : {-100 * timeDelta, +100 * timeDelta})
        {
            ...
            env (offer (bob, JBC (50), USD (50)));
            env (offer (bob, JBC (100), USD (50)));

            TER const expectedResult = closeTime < fix1141Time ()
                ? TER {tecPATH_DRY}
                : TER {tesSUCCESS};
            env (pay (alice, carol, USD (100)), path (~USD), sendmax (JBC (100)),
                txflags (tfNoJBCoinDirect | tfPartialPayment | tfLimitQuality),
                ter (expectedResult));
    ...
    static JBCAmount reserve(jtx::Env& env, std::uint32_t count)
    {
    ...
    void testSelfFundedJBCEndpoint (bool consumeOffer, FeatureBitset features)
    {
        // Test that the deferred credit table is not bypassed for
        // JBCEndpointSteps. If the account in the first step is sending JBC and
        // that account also owns an offer that receives JBC, it should not be
        // possible for that step to use the JBC received in the offer as part
        // of the payment.
        testcase("Self funded JBCEndpoint");
        ...
        env(offer(alice, JBC(50000), USD(10)));
        ...
        env(pay(alice, alice, toSend), path(~USD), sendmax(JBC(20000)),
            txflags(tfPartialPayment | tfNoJBCoinDirect));
    }
    ...
    void testUnfundedOffer (bool withFix, FeatureBitset features)
    {
        ...
        using namespace jtx;
        {
            ...
            env.fund(JBC(100000), alice, bob, gw);
            ...
            env(pay(alice, bob, tinyAmt1), path(~USD),
                sendmax(drops(9000000000)), txflags(tfNoJBCoinDirect));

            if (withFix)
                BEAST_EXPECT(!isOffer(env, gw, JBC(0), USD(0)));
            else
                BEAST_EXPECT(isOffer(env, gw, JBC(0), USD(0)));
        }
        {
            ...
            env(pay(alice, bob, drops(9000000000)), path(~JBC),
                sendmax(USD(1)), txflags(tfNoJBCoinDirect));

            if (withFix)
                BEAST_EXPECT(!isOffer(env, gw, USD(0), JBC(0)));
            else
                BEAST_EXPECT(isOffer(env, gw, USD(0), JBC(0)));
        }
    }
    ...
    testReexecuteDirectStep(FeatureBitset features)
    {
        ...
        BEAST_EXPECT(!getNoJBCoinFlag(env, gw, alice, usdC));
        ...
        env(pay(alice, bob, JBC(10000)), path(~JBC), sendmax(USD(100)),
            txflags(tfPartialPayment | tfNoJBCoinDirect));
    }
    ...
    testRIPD1443(bool withFix)
    {
        ...
        env.fund(JBC(100000000), alice, nojbcoin(bob), carol, gw);
        env.trust(gw["USD"](10000), alice, carol);
        env(trust(bob, gw["USD"](10000), tfSetNoJBCoin));
        env.trust(gw["USD"](10000), bob);
        env.close();

        // set no jbcoin between bob and the gateway

        env(pay(gw, alice, gw["USD"](1000)));
        env.close();

        env(offer(alice, bob["USD"](1000), JBC(1)));
        env.close();

        env(pay(alice, alice, JBC(1)), path(gw, bob, ~JBC),
            sendmax(gw["USD"](1000)), txflags(tfNoJBCoinDirect),
        ...
        env(offer(alice, JBC(1000), bob["USD"](1000)));
        env.close();

        env(pay (carol, carol, gw["USD"](1000)), path(~bob["USD"], gw),
            sendmax(JBC(100000)), txflags(tfNoJBCoinDirect),
            ter(withFix ? TER {tecPATH_DRY} : TER {tesSUCCESS}));
        env.close();
    ...
    testRIPD1449(bool withFix)
    {
        ...
        // pay alice -> jbc -> USD/bob -> bob -> gw -> alice
        // set no jbcoin on bob's side of the bob/gw trust line
        ...
        env.fund(JBC(100000000), alice, bob, carol, gw);
        env.trust(USD(10000), alice, carol);
        env(trust(bob, USD(10000), tfSetNoJBCoin));
        ...
        env(offer(carol, JBC(1), bob["USD"](1000)));
        env.close();

        env(pay(alice, alice, USD(1000)), path(~bob["USD"], bob, gw),
            sendmax(JBC(1)), txflags(tfNoJBCoinDirect),
            ter(withFix ? TER {tecPATH_DRY} : TER {tesSUCCESS}));
        env.close();
    }
    ...
    testSelfPayLowQualityOffer (FeatureBitset features)
    {
        ...
        // JBC than the offering account held.  This unit test reproduces
        // that failing case.
    ...
    void testWithFeats(FeatureBitset features)
    {
        ...
        testSelfFundedJBCEndpoint(false, features);
        testSelfFundedJBCEndpoint(true, features);
...
BEAST_DEFINE_TESTSUITE_PRIO(Flow,app,jbcoin,2);
BEAST_DEFINE_TESTSUITE_MANUAL_PRIO(Flow_manual,app,jbcoin,4);
