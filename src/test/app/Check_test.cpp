...
class Check_test : public beast::unit_test::suite
{
    ...
    void testEnabled()
    {
        ...
        Account const alice {"alice"};
        {
            ...
            env.fund (JBC(1000), alice);

            uint256 const checkId {
                getCheckIndex (env.master, env.seq (env.master))};
            env (check::create (env.master, alice, JBC(100)),
                ter(temDISABLED));
            env.close();

            env (check::cash (alice, checkId, JBC(100)),
                ter (temDISABLED));
            env.close();
        ...
        }
        {
            ...
            env (check::cash (alice, checkId1, JBC(100)));
            env.close();
    ...
    void testCreateValid()
    {
        ...
        STAmount const startBalance {JBC(1000).value()};
            ...
            [&env, &USD, this] (Account const& from, Account const& to)
        {
            ...
            env (check::create (from, to, JBC(2000)));
            env.close();
    ...
    void testCreateInvalid()
    {
        ...
        // Check to self.
        env (check::create (alice, alice, JBC(10)), ter (temREDUNDANT));
        env.close();
    ...
    void testCashJBC()
    {
        // Explore many of the valid ways to cash a check for JBC.
        testcase ("Cash JBC");
        ...
        env.fund (startBalance, alice, bob);
        {
            ...
            env (check::cash (bob, chkId, JBC(10)));
            env.close();
            env.require (balance (alice,
                startBalance - JBC(10) - drops (baseFeeDrops)));
            ...
            env (pay (env.master, alice, JBC(10) + drops (baseFeeDrops)));
            env (pay (bob, env.master,   JBC(10) - drops (baseFeeDrops * 2)));
            env.close();
    ...
    void testCashIOU ()
    {
        ...
        IOU const USD {gw["USD"]};
        {
            ...
            env.fund (JBC(1000), gw, alice, bob);
    ...
    void testCashInvalid()
    {
        ...
        env.fund (JBC(1000), gw, alice, bob, zoe);
        ...
        env (check::create (alice, bob, JBC(10)), expiration (env.now() + 1s));
        env.close();
        ...
        // Same set of failing cases for both IOU and JBC check cashing.
        ...
        failingCases (chkIdX, JBC(10));
        failingCases (chkIdU, USD(20));

        // Verify that those two checks really were cashable.
        env (check::cash (bob, chkIdU, USD(20)));
        env.close();
        env (check::cash (bob, chkIdX, check::DeliverMin (JBC(10))));
        verifyDeliveredAmount (env, JBC(10));

        // Try to cash an expired check.
        env (check::cash (bob, chkIdExp, JBC(10)), ter (tecEXPIRED));
        env.close();
    ...
    void testCancelValid()
    {
        ...
        env (check::create (alice, bob, JBC(10)), expiration (env.now()+600s));
        env.close();
    ...
    void testFix1623Enable ()
    {
        ...
        auto testEnable = [this] (FeatureBitset const& features, bool hasFields)
        {
            ...
            uint256 const chkId {getCheckIndex (alice, env.seq (alice))};
            env (check::create (alice, bob, JBC(200)));
            env.close();

            env (check::cash (bob, chkId, check::DeliverMin (JBC(100))));
...
BEAST_DEFINE_TESTSUITE (Check, tx, jbcoin);
