...
We do not recommend Windows for jbcoind production use at this time. Currently,
...
modification, build jbcoind under Visual Studio, and run the unit tests you will
...
Any version of Visual Studio 2017 may be used to build jbcoind. The **Visual
...
Building jbcoind requires **protoc.exe** version 2. Version 3 is not currently
...
([alternate
link](https://github.com/jbcoin/Downloads/raw/gh-pages/protoc/2.5.1/protoc.exe))
precompiled Windows executable from the [JBCoin
Organization](https://github.com/jbcoin).
...
* NOTE: Since jbcoind links statically to OpenSSL, it does not matter where the
  OpenSSL .DLL files are placed, or what version they are. jbcoind does not use
...
The jbcoind application is linked statically to the standard runtimes and
...
process and clone `git@github.com:jbcoin/jbcoind.git`. Otherwise follow this
...
above. Navigate to the directory where you want to clone jbcoind (git bash uses
...
listed on [jbcoind's GitHub page](https://github.com/jbcoin/jbcoind/releases). 
...
5. Select either the `jbcoind.exe` (unity) or `jbcoind_classic.exe` (non-unity)
   ...
   jbcoind repo, e.g. `datagen.vcxproj`. Please ignore them.
...
```
.\build\x64-Release\Release\jbcoind(_classic).exe
```
or
```
.\build\x64-Debug\Debug\jbcoind(_classic).exe
```
...
execute the following commands within your `jbcoind` cloned repository:
...
Navigate to the `build\cmake` folder created above and select the `jbcoind.sln`
file. You can then choose whether to build the `Debug` or `Release` solution
configuration. Within the **Solution Explorer**, selected either the `jbcoind`
(unity build) project or the `jbcoind_classic` (non-unity) project, and
right-click to build.

The executable will be in 
```
.\build\cmake\Release\jbcoind(_classic).exe
```
 or
````
.\build\cmake\Debug\jbcoind(_classic).exe
````
where these paths are relative to your cloned git repository.

# Unit Test (Recommended)

`jbcoind` builds a set of unit tests into the server executable. To run these
unit tests after building, pass the `--unittest` option to the compiled
`jbcoind` executable. The executable will exit with summary info after running
the unit tests.
