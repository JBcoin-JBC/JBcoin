...
//
github_api = 'https://api.github.com/repos/jbcoin/jbcoind'
...
"""
    ...
    stage ('Parallel Build') {
        ...
        for (int index = 0; index < variants.size(); index++) {
            ...
            def node_type =
                (compiler == 'msvc') ? 'jbcoind-win' : 'jbcoind-dev'
            ...
            builds[bldlabel] = {
                node(node_type) {
                    ...
                    withCredentials(
                        [string(
                            credentialsId: 'JBCOIND_CODECOV_TOKEN',
        ...
        builds['rpm'] = {
            node('docker') {
                def bldlabel = 'rpm'
                def remote =
                    (git_fork == 'jbcoin') ? 'origin' : git_fork

                withCredentials(
                    [string(
                        credentialsId: 'JBCOIND_RPM_ROLE_ID',
                        variable: 'ROLE_ID')])
                {
                    withEnv([
                        'docker_image=artifactory.ops.jbcoin.com:6555/jbcoind-rpm-builder:latest',
...
# Vault Steps
SECRET_ID=$(cat /.vault/jbcoind-build-role/secret-id)
export VAULT_TOKEN=$(/usr/local/jbcoin/ops-toolbox/vault/vault_approle_auth -r ${ROLE_ID} -s ${SECRET_ID} -t)
/usr/local/jbcoin/ops-toolbox/vault/vault_get_sts_token.py -r jbcoind-build-role
...
docker run --rm \
-v $PWD/rpm-out:/opt/jbcoind-rpm/out \
...
rpm -i /opt/jbcoind-rpm/*.rpm
rc=$?; if [[ $rc != 0 ]]; then
  error "error installing rpms"
fi

/opt/jbcoin/bin/jbcoind --unittest
rc=$?; if [[ $rc != 0 ]]; then
  error "jbcoind --unittest failed"
fi

/opt/jbcoin/bin/validator-keys --unittest
rc=$?; if [[ $rc != 0 ]]; then
...
docker run --rm \
-v $PWD/rpm-out/rpm-main:/opt/jbcoind-rpm \
-v $PWD:/opt/jbcoind --entrypoint /opt/jbcoind/test_rpm.sh \
centos:latest
'''
}
